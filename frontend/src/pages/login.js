...
export default function Login(props) {
  ...
  const handleCallback = async (response) => {
    ...
    try{
      if (response.credential) {
        const res = await axios.post('https://capstone-77b48.web.app/auth/google', {token: response.credential}, {withCredentials: true});
  ...
  const handleLogin = async () => {
    try {
      const res = await axios.post('https://capstone-77b48.web.app//auth/login', { username: username, password: password });
      if (res.data.success) {
